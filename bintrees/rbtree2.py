...
class TreeNode(NilNode):
    def __init__(self, key, value, parent, color):
        super(TreeNode, self).__init__(parent, color)
        self.key = key
        self.value = value
...
class RBTree(BaseTree):
    ...
    def new_node(self, key, value, parent, color):
        self._count += 1
        node = TreeNode(key, value, parent, color)
        node.left = self.nil_node(node)
        node.right = self.nil_node(node)
        return node

    def nil_node(self, parent):
        return NilNode(parent, _BLACK)
